...
public class PositionControlLoop extends LinearSystemControlLoop<N2, N1, N1> {
  protected PositionControlLoop(
      ...
      Matrix<N1, N2> C,
      Matrix<N1, N1> D,
      Matrix<N2, N1> modelSTDDev,
      double measPositionStdDev,
      Vector<N2> maxError,
    ...
    super(
        ...
        modelSTDDev,
        VecBuilder.fill(measPositionStdDev),
        latencyCompensation,
        clampFunction,
        dtSeconds);
  }

  public double setReferenceAndPredict(
      double setpointPos, double setpointVel, double systemOutput) {
    return setReferenceAndPredict(
            VecBuilder.fill(setpointPos, setpointVel), VecBuilder.fill(systemOutput))
        .get(0, 0);
  }
  ...
  public static class PositionControlLoopBuilder
      extends VelocityControlLoop.VelocityControlLoopBuilder {
    ...
    public PositionControlLoopBuilder stateMatrices(
        Matrix<N2, N2> A, Matrix<N2, N1> B, Matrix<N1, N2> C, Matrix<N1, N1> D) {
      this.A = A;
      this.B = B;
      this.C = C;
      this.D = D;
    ...
    public PositionControlLoopBuilder standardDeviations(
        double modelPos, double modelVel, double meas) {
    ...
    public PositionControlLoop buildPositionControlLoop() {
      ...
      if (kS > 0.005)
        ...
        clampFunction =
            (u) ->
                VecBuilder.fill(MathUtil.clamp(u.get(0, 0), -maxControlEffort, maxControlEffort));
      return new PositionControlLoop(
          A,
          B,
          C,
          D,
    ...
    public PositionControlLoop buildArmControlLoop() {
      PositionControlLoop loop =
          new PositionControlLoop(
              A,
              B,
              C,
              D,
