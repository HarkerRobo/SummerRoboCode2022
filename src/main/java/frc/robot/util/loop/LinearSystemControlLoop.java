...
public class LinearSystemControlLoop<States extends Num, Inputs extends Num, Outputs extends Num> {
    ...
    public LinearSystemControlLoop(Matrix<States, States> A, Matrix<States, Inputs> B, Matrix<Outputs, States> C, Matrix<Outputs, Inputs> D, 
        Nat<States> states, Nat<Outputs> outputs, Vector<States> qelms, Vector<Inputs> relms, Matrix<States, N1> stateStdDevs, 
        Matrix<Outputs, N1> measurementStdDevs, double latencyCompensation, Function<Matrix<Inputs, N1>, Matrix<Inputs, N1>> clampFunction, double dtSeconds) {

        LinearSystem<States, Inputs, Outputs> plant = new LinearSystem<>(A, B, C, D);
        loop = new LinearSystemLoop<>(plant, new LinearQuadraticRegulator<>(plant, qelms, relms, dtSeconds), 
            new KalmanFilter<>(states, outputs, plant, stateStdDevs, measurementStdDevs, dtSeconds), clampFunction, dtSeconds);
        loop.getController().latencyCompensate(plant, dtSeconds, latencyCompensation);
        this.dtSeconds = dtSeconds; 
    }

    protected Matrix<Inputs, N1> setReferenceAndPredict(Matrix<States, N1> setpoint, Matrix<Outputs, N1> systemOutput) {
